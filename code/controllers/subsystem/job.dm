...
/datum/controller/subsystem/job/proc/not_enough_kids()
	return GetJob(overflow_role).current_positions < CONFIG_GET(number/fnaf_minimum_kids)

/datum/controller/subsystem/job/proc/SetupOccupations(faction = "FNAF")
	occupations = list()
...
/datum/controller/subsystem/job/proc/AssignRole(mob/dead/new_player/player, rank, latejoin = FALSE)
	...
	if(player && player.mind && rank)
		...
		if(job.required_playtime_remaining(player.client))
			return FALSE
		if(rank != overflow_role && not_enough_kids())
			return FALSE
...
/datum/config_entry/number/fnaf_minimum_kids
...
/datum/controller/subsystem/job/proc/GiveRandomJob(mob/dead/new_player/player)
	...
	for(var/datum/job/job in shuffle(occupations))
		...
		if(istype(job, GetJob(SSjob.overflow_role)) && !not_enough_kids()) // We don't want to give him assistant, that's boring!
			continue
...
/datum/controller/subsystem/job/proc/DivideOccupations()
	...
	//Not enough kids, sorry.
	JobDebug("DO, Enough Kids Check")
	while(unassigned.len && not_enough_kids())
		JobDebug("DO, Not enough kids")
		AssignRole(pick(unassigned), SSjob.overflow_role)
