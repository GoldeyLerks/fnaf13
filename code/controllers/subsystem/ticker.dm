...
/datum/controller/subsystem/ticker/fire()
	switch(current_state)
		if(GAME_STATE_STARTUP)
			...
			to_chat(world, file2text("html/startup_notice.html"))
...
/datum/controller/subsystem/ticker/proc/setup()
	...
	if(!GLOB.Debug2)
		if(!can_continue)
			...
			to_chat(world, "<B>Error setting up [GLOB.master_mode] ([mode.setup_error]).</B> Reverting to pre-game lobby.")
			QDEL_NULL(mode)
			SSjob.ResetOccupations()
			return 0
	else
...
/datum/controller/subsystem/ticker/proc/equip_characters()
	...
	for(var/mob/dead/new_player/N in GLOB.player_list)
		...
		if(istype(player) && player.mind && player.mind.assigned_role)
			if(player.mind.assigned_role == "Manager")
				captainless=0
	...
	if(captainless)
		for(var/mob/dead/new_player/N in GLOB.player_list)
			if(N.new_character)
				to_chat(N, "Managership not forced on anyone.")
			CHECK_TICK
...
/datum/controller/subsystem/ticker/proc/send_news_report()
	...
	switch(news_report)
		...
		if(STATION_NUKED)
			news_message = "[station_name()] activated its self destruct device for unknown reasons. Attempts to clone the Manager so he can be arrested and executed are underway."
		if(CLOCK_SUMMON)
